        ...
        const Mat desc_bg, const Point2f center)
{
	...
	this->num_fg_points = desc_fg.rows;
	...
	cout << "datasize" << database.rows << endl;
...
void Matcher::matchupdata(const Mat newdata, const vector<int> & classes_pt)
{
	if (database.cols == newdata.cols)
	{
	
	//vector<vector<DMatch> > matches;
	vconcat(database, newdata, database);
	classes.insert(classes.end(), classes_pt.begin(), classes_pt.end());

	/*	bfmatcher->knnMatch(descriptors, newdata, matches, 2);
